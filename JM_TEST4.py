...
most_popular_name_row = df_aggregated.loc[df_aggregated['Count'].idxmax()]
most_popular_name = most_popular_name_row['Name']
most_popular_name_count = most_popular_name_row['Count']

# Yearly trends for the most popular name
df_yearly_trends = df_top_baby_names_yr[df_top_baby_names_yr['Name'] == most_popular_name]

# Display the most popular name and its total occurrences
st.title('Most Popular Baby Name in the Last 100 Years')
st.write(f"The most popular name is **{most_popular_name}** with a total of **{most_popular_name_count}** occurrences.")
...
yearly_trends_chart = alt.Chart(df_yearly_trends).mark_line(point=True).encode(
    ...
    width=800,
    height=400,
    title=f"Yearly Trends of the Name {most_popular_name}"
)
